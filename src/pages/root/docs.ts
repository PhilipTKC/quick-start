...
export class Docs implements IRouteableComponent {
    ...
    async loading(parameters: Parameters) {
        ...
        this.tableOfContents = await this.documentService.retrieveTableOfContents(documentId as string);
    }

    attached() {
        this.ea.subscribeOnce("au:router:navigation-end", async (route: any) => {
            if (route.navigation.instruction === "docs" || route.navigation.instruction === `/${this.id}/docs`) {
